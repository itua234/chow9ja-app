...
import { get_flags } from "@/services/api";
...
const Signup = () => {
    ...
    const Signup = async () => {
        ...
        Keyboard.dismiss(); // Dismiss the keyboard only once
        setLoading(true);
        dispatch(clearErrors());
        handleMessage('');
        try{
            await new Promise(resolve => setTimeout(resolve, 100)); // Create a delay
            const res: AxiosResponse = await register(payload);
            const email = res.data?.results;
            if(email){
                router.push({
                    pathname: "/verify-email",
                    params: {email: email}
            ...
            if (error.response) {
                ...
                handleMessage("An unexpected error occurred. Please try again.");
            }
        } finally {
            setLoading(false);
        }
